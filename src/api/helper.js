...
export const authenticatedPost = async (urlPath, params, jsonStringifyParams) => {
    const requestInit = {
        method: 'POST',
        headers: {
...
export const unauthenticatedGet = async (urlPath, external) => {
    ...
    const url = external ? urlPath : `${Config.API_BASE}${urlPath}`;
    const res = await fetch(url, requestInit);
    return await handleResponse(res, url, requestInit);
};
export const authenticatedPut = async (urlPath, params, jsonStringifyParams) => {
    const requestInit = {
        method: 'PUT',
        headers: {
            Accept: 'application/json',
...
export const authenticatedGet = async (urlPath, params, extraPath) => {
    ...
    const url = `${Config.API_BASE}${urlPath}?${query}${extraPath}`;
    showRequestAndResponse &&
        console.log('request curl: ', fetchToCurl(url, requestInit));
    const res = await fetch(url, requestInit);
    return await handleResponse(res, url, requestInit);
};
export const authenticatedDelete = async (urlPath, params, jsonStringifyParams) => {
    const requestInit = {
        method: 'DELETE',
        headers: {
            Accept: 'application/json',
...
export const unauthenticatedPost = async (urlPath, params, jsonStringifyParams) => {
    const requestInit = {
        method: 'POST',
        headers: {
            Accept: 'application/json',
